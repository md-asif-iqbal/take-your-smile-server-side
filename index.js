...
async function run() {
  try {
    ...
    const SummeryCollection = client
      .db("HomePageFeathers")
      .collection("Summery");
    ...
    app.put("/user/:email", async (req, res) => {
      ...
      const token = jwt.sign(filter, process.env.ACCESS_TOKEN_SECRET, {
        expiresIn: "1d",
      });
    ...
    app.post("/recentEvents", async (req, res) => {
      const query = req.body;
      const review = await RecentEventCollection.insertOne(query);
      res.send(review);
    });
